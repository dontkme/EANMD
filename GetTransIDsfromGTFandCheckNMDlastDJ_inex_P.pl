...
while(defined (our $inrow=<ANNOT>)){    ### Get CDS/Coding gene transcripts id.
    @tmp=();
    @tmp = split (/\t/,$inrow);
...
our (%CDSseqs, %exonseqs, %CDSstartPos, %CDSendPos, %exonstartPos, %exonendPos, %CDSsLength, %exonsLength, %accCDSlength, %reverseaccCDSlength);
 ##### Makedb of exons, CDSs with sortkey.
...
close OUT; # Output the CDSs and exons sequences fasta with the key as name.
...
# while(defined(our $inputline = <INPUTLIST>)){   # Skip the OUTSEQ. 2021-04-28
#     if ($inputline =~ m/^\#/) {next;}
...
# close INPUTLIST;  # Skip the OUTSEQ. 2021-04-28 END.
...
my @Lines = <INPUTLIST>; # Get Input List line numbers.
my $Lines = @Lines;
close INPUTLIST;
our $splitLines = 1;
$splitLines = int(($Lines + $MAX_processes)/$MAX_processes); # Get split lines number if we want to split to $MAX_processes files.
print "Input List file: $inputlist has $Lines line(s).\nSplit it to $MAX_processes parts, $splitLines lines each part.\n";

our %handles;  ### Split the inputlist file.
foreach(1..$MAX_processes){
    my $outfile = $inputlist.".$_.tmp";
    open $handles{$_}, ">$outfile" or die ("[-] Error: Can't open or create temp file $outfile\n");
}

open INPUTLIST, "< $inputlist" or die ("[-] Error: Can't open list file: $inputlist.\n");
my $linenumbercount=0;
while (my $tmpinputLine = <INPUTLIST>){
    $linenumbercount++;
    for(my $tmpflieID=1; $tmpflieID <= $MAX_processes; $tmpflieID++){
        if ($linenumbercount>= ($tmpflieID - 1) * $splitLines and $linenumbercount < $tmpflieID * $splitLines){
            print {$handles{$tmpflieID}} "$tmpinputLine";
        }
    }
}
close INPUTLIST;
...
for (my $pid =1; $pid<=$MAX_processes; $pid++){     ####################################### Start loop!!!!!!!!!!!!!!!!!!!!!!!!!!
    $pm->start and next;
    my $Pinputfile = $inputlist.".$pid.tmp";
    open INPUTLISTP, "< $Pinputfile" or die ("[-] Error: Can't open temp list file: $Pinputfile.\n");
    open OUTUSDSTRANSID, "> $opfn.outUSDStransid.txt.$pid.tmp" or die ("[-] Error: Can't open or create temp $opfn.outUSDStransid.txt.$pid.tmp\n");
    open OUTUSSEDSTRANSID, "> $opfn.outUSSEDStransid.txt.$pid.tmp" or die ("[-] Error: Can't open or create temp $opfn.outUSSEDStransid.txt.$pid.tmp\n");

    open OUTTRANSID, "> $opfn.outtransid.txt.$pid.tmp" or die ("[-] Error: Can't open or create temp $opfn.outtransid.txt.$pid.tmp\n");
    open OUTSETRANSID, "> $opfn.outSEtransid.txt.$pid.tmp" or die ("[-] Error: Can't open or create temp $opfn.outSEtransid.txt.$pid.tmp\n");
    open OUTUSTRANSID, "> $opfn.outUStransid.txt.$pid.tmp" or die ("[-] Error: Can't open or create temp $opfn.outUStransid.txt.$pid.tmp\n");
    open OUTDSTRANSID, "> $opfn.outDStransid.txt.$pid.tmp" or die ("[-] Error: Can't open or create temp $opfn.outDStransid.txt.$pid.tmp\n");
    ...
    while(defined(our $inputline = <INPUTLISTP>)){
        ...
        foreach my $tmptransid(@USSEDStransid){   ##### 1st For. !!!!!
            ...
            if ($transPM eq "+"){   ########## First analysis Plus strand.
                ...
                our $originexonRmLastexonseqLen = 0; ###
                $originexonRmLastexonseqLen = $originexonseqsLen - length($exonseqs{$tmptransid}[$transexonnumbers]); ### 2021.05.06 accumulate penultimate exon length.
                ...
                our $originCDSseqs = ""; #### Original reference CDS sequence and length. !!!! just suitable for + strand. !!!!
                ...
                our $originexonCDSseqs = "";
                our $originexonCDSseqsLen = 0;
                our $originCDSexonseqs = "";
                our $originCDSexonseqsLen = 0;

                for (my $i1 =1; $i1<= $transexonnumbers; $i1++){ #### Get exons + CDS end seq and seq length.
                    if ($i1 < $stopcodon_exonnumber){
                        $originexonCDSseqs .= $exonseqs{$tmptransid}[$i1];
                    }elsif($i1 == $stopcodon_exonnumber){
                        $originexonCDSseqs .= $CDSseqs{$tmptransid}[$i1];
                    }
                ...
                our $originCDSexonRmLastexonseqLen = 0;  ####2. report value
                 
                $originCDSexonRmLastexonseqLen = $originCDSexonseqsLen - length($exonseqs{$tmptransid}[$transexonnumbers]); ### Get penultimate CDS + exons seq length.
                ...
                if ($setransexonid < $startcodon_exonnumber){ # Start tell SE position. 5UTR Next!!!
                    ...
                    for (my $ii = 1; $ii <= $transexonnumbers; $ii++){
                        ...
                        $tmpAccrmSEExons[$ii] = length($removeSEexonsseq);   ####### Get Accumulated exon length.
                    ...
                    my $LongestORF_AA_start = "";
                    my $LongestORF_AA_end = "";
                    my $LongestORF_DNA_start = "";
                    my $LongestORF_DNA_end = "";
                    my $flagATG = "ATG";

                    if ($tmpLongestORF_AA ne ""){
                        $LongestORF_AA_start = $tmpORFseqPos{$tmpLongestORF_AA}[0];
                        $LongestORF_AA_end = $tmpORFseqPos{$tmpLongestORF_AA}[1];
                        $LongestORF_DNA_start = $tmpORFseqPos{$tmpLongestORF_AA}[2];
                        $LongestORF_DNA_end = $tmpORFseqPos{$tmpLongestORF_AA}[3];
                    ...
                    #clac the distence of SE-removed exons 1st stop to last junction.
                    my $flag4 ="-";
                    my $flag3= "-";
                    my $final_1st_stopcodon_exon_number ="-";
                    my $NewstopcodonPos =  $LongestORF_DNA_end;
                    my $flag1 = "-";

                    if ($tmpLongestORF_AA !~ /_/){
                        $flag1 = "No stop_codon";
                    }

                    if ($tmpLongestORF_AA eq ""){
                        $flag1 = "No M_ AA";
                    }
                    my $tmpdjlast ="-";


                    #  $tmpdjlast = $tmpAccrmSEExons[$transexonnumbers-1] - $LongestORF_DNA_end;
                    if ($tmpLongestORF_AA ne ""){
                        if ($setransexonid == $transexonnumbers-1){
                            $tmpdjlast = $tmpAccrmSEExons[$transexonnumbers-2] - $LongestORF_DNA_end;
                        }else {
                            $tmpdjlast = $tmpAccrmSEExons[$transexonnumbers-1] - $LongestORF_DNA_end;
                        }
                ...
                # initialize removed SE exons.
                our $originCDSexonseqs = "";
                our $originCDSexonseqsLen = 0;
                # our $originCDSexonRmLastexonseqLen = 0;
                # our $originCDSdj=0;
                # our $flagOriginNMD="-";

                for (our $x = $transexonnumbers; $x >=1; $x--){ # Original Exon sequence and length.
                    $originexonseqs .= $exonseqs{$tmptransid}[$x];
                }
                ...
                #### Removed-SE  transcript's exons sequence and length.
                for (our $y = $transexonnumbers; $y > $setransexonid; $y--){ 
                    $removeSEexonseqs .= $exonseqs{$tmptransid}[$y];
                }
                ...
                our $originCDSseqs = ""; #### Original reference CDS sequence and length. !!!!

                for (our $x = $stopcodon_exonnumber; $x >= $startcodon_exonnumber; $x--){                 
                    $originCDSseqs .= $CDSseqs{$tmptransid}[$x];
                }

                our $originCDSseqsLen = length($originCDSseqs);
                ...
                $originCDSexonRmLastexonseqLen = $originCDSexonseqsLen - length($exonseqs{$tmptransid}[1]);
                ...
                if ($setransexonid > $stopcodon_exonnumber){ # This stopcodon_exonnumber is the startcondon_exonnumber in - shrand. # 5UTR pass.
                    ...
                    our $removeSECDSseqs = "";
                    $removeSECDSseqs .= $CDSseqs{$tmptransid}[$stopcodon_exonnumber];
                    $accCDSlength{$tmptransid}[$stopcodon_exonnumber] = length($removeSECDSseqs);
                    ...
                    if (!defined($removeSEexonseqsAA_1stPos)){ ### To skip warnings of not initailized @.
                        $removeSEexonseqsAA_1stPos= "Null";
                        $removeSEexonseqsAA_allPos = "Null";
                    }                    
                    ...
                    if ($flag1 eq "Upstream stop_codon"){   ### Tell NMD.
                        ...
                        for (my $a = $stopcodon_exonnumber; $a >= 1; $a--){ #### scan CDS + exons.
                            ...
                            if ($a == $stopcodon_exonnumber){   # Get 1st CDS and rest exons length as $tmplen.
                                $tmplen = $CDSsLength{$tmptransid}[$a];
                            }else{
                                $tmplen = $exonsLength{$tmptransid}[$a];  
                            }
                            ...
                            if (($NewstopcodonPos >= $accCDSlength{$tmptransid}[$a] - $tmplen) and ($NewstopcodonPos <= $accCDSlength{$tmptransid}[$a])){ # search 1st stopcodon length range.
                                ...
                                if ($setransexonid == 2){ ##### tell penultimate exon.
                                ...
                                if ($final_1st_stopcodon_exon_number == 1){ #### Last exonnumber: 1
                                    $flag3 = "Last_exon";                            
                                }elsif($tmpdj>= $dj){
                                    $flag3 = "NMD";  ############### NMD dj <= 50 rules.
                                }
                ...
                ##### Nedd additional codes.
                    ...
                    if ($tmpLongestORF_AA ne ""){
                        if ($setransexonid == 2){
                            $tmpdjlast = $tmpAccrmSEExons[3] - $LongestORF_DNA_end;
                        }else {
                            $tmpdjlast = $tmpAccrmSEExons[2] - $LongestORF_DNA_end;
                        }
                    ...
                    our $removeSECDSseqs = $CDSseqs{$tmptransid}[$stopcodon_exonnumber];
                    $accCDSlength{$tmptransid}[$stopcodon_exonnumber] = length($removeSECDSseqs);
        ...
        foreach my $tmptransid(@noSEtransid){   
            ...
            my $SEtransexonumber = 0;
            my $UStransexonnumber = 0;
            my $DStransexonnumber = 0;
            # $SEseq
            ...
            foreach my $exonkey(@tmpCDSkeys){  #search this transcript's CDS keys. avoid missing $DStransexonnumber, if it is a boundary of CDS and UTR. 2021/05/06
                # say $SEpos;
                if ($exonkey =~ m/$USpos/){
                    # say $SEkey;
                    # $setransexonid=$key2exonnumber{$SEkey};
            ...
            if ($transPM eq "+"){
            ...
            }elsif($transPM eq "-"){ ##### Because in MM10 ensemble GTF, exons number is count by +. The  - strand exons start with the largest number of exon No. So using $DStransexonnumber as boundary. Switch stopcodon_exonnumber and startcodon_exonnumber, $UStransexonnumber and $DStransexonnumber.
                                    ######################### 2021-04-26
                ...
                if ($stopcodon_exonnumber < $DStransexonnumber){ # $stopcodon_exonnumber is the startcodon_exonnumber in minus strand.
                    ...
                    for (my $tmpi = $stopcodon_exonnumber; $tmpi > $SEtransexonumber; $tmpi--){
                        my $tmpseq="";
                        if ($tmpi == $stopcodon_exonnumber){
                            $tmpseq = $CDSseqs{$tmptransid}[$tmpi];
                        }else{
                            $tmpseq = $exonseqs{$tmptransid}[$tmpi];
                        }
                    ...
                    if($SEtransexonumber - int($SEtransexonumber)== 0.5){ 
                        ...
                        for (my $tmpj = $SEtransexonumber - 0.5; $tmpj >= 1; $tmpj-- ){ ### Add rest exons.
                            my $tmpseq="";
                            $tmpseq = $exonseqs{$tmptransid}[$tmpj];
                            $tmpOriginCDS .=$tmpseq;
                            $tmpAddCDS .= $tmpseq;
                        ...
                        for (my $tmpj = $SEtransexonumber - 1; $tmpj >=1; $tmpj-- ){ ### Add rest exons.
                            my $tmpseq="";
                            $tmpseq = $exonseqs{$tmptransid}[$tmpj];
                            $tmpOriginCDS .=$tmpseq;
                            $tmpAddCDS .= $tmpseq;
                    ...
                    my $originFullCDS="";
                    my $originFullCDSlen="";
                    for (my $tmpk =$stopcodon_exonnumber; $tmpk >= $startcodon_exonnumber; $tmpk --){
                        $originFullCDS .= $CDSseqs{$tmptransid}[$tmpk];
                ...
                # $originexonCDSseqsLen = length($originexonCDSseqs);
                    ...
                    if ($SEtransexonumber == 2 or $SEtransexonumber == 1.5){  ########## Tell last Junction.
                        $lastJpos = $tmpaccAddCDSlen{$SEtransexonumber};
                    }else{
                        $lastJpos = $tmpaccAddCDSlen{2};
                    }
...
for(my $pid =1; $pid<=$MAX_processes;$pid++){
    my $outfile = $inputlist.".$pid.tmp";
    unlink "$outfile";  # not Now.

    open OUTUSDSTRANSID, "$opfn.outUSDStransid.txt.$pid.tmp";
    open OUTUSSEDSTRANSID, "$opfn.outUSSEDStransid.txt.$pid.tmp";

    print OUTUSDSTRANSIDALL <OUTUSDSTRANSID>;
    print OUTUSSEDSTRANSIDALL <OUTUSSEDSTRANSID>;

    close OUTUSDSTRANSID; 
    close OUTUSSEDSTRANSID;

    unlink "$opfn.outUSDStransid.txt.$pid.tmp";
    unlink "$opfn.outUSSEDStransid.txt.$pid.tmp";

    open OUTTRANSID, "$opfn.outtransid.txt.$pid.tmp";
    open OUTSETRANSID, "$opfn.outSEtransid.txt.$pid.tmp";
    open OUTUSTRANSID, "$opfn.outUStransid.txt.$pid.tmp";
    open OUTDSTRANSID, "$opfn.outDStransid.txt.$pid.tmp";

    print OUTTRANSIDALL <OUTTRANSID>;
    print OUTSETRANSIDALL <OUTSETRANSID>;
    print OUTUSTRANSIDALL <OUTUSTRANSID>;
    print OUTDSTRANSIDALL <OUTDSTRANSID>;
    ...
    unlink "$opfn.outtransid.txt.$pid.tmp";
    unlink "$opfn.outSEtransid.txt.$pid.tmp";
    unlink "$opfn.outUStransid.txt.$pid.tmp";
    unlink "$opfn.outDStransid.txt.$pid.tmp";

}

close OUTTRANSIDALL;
close OUTSETRANSIDALL;
close OUTUSTRANSIDALL;
close OUTDSTRANSIDALL;
